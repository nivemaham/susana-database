...
def pandas_translate(v):
    ...
    return(translator.translate(v, src='fr', dest="en").text)

translated = df.limit(100).withColumn('group_result', pandas_translate(df.group_lib)).toPandas()
...
## icdmap = sql("""
## (select concept_id_1, concept_id_2, 'Maps To' as relationship_id, current_date() valid_start_date, '2099-01-01' valid_end_date 
## from mapped) 
## UNION ALL 
## (select concept_id_2, concept_id_1, 'Mapped From' as relationship_id, current_date() valid_start_date, '2099-01-01' valid_end_date 
## from mapped)
## """)
## 
## # ENRICH MAPPING
